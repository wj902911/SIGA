#!/usr/bin/env python3
"""Plot electrode electric potential against length-scale/height ratio."""
...
MOMENT_MN_UM_TO_MICRONEWTON_UM = 1.0e3
    ...
    with output_path.open("w", encoding="utf-8", newline="") as f:
        ...
        writer.writerow(
            [
                ...
                "young_modulus_GPa",
                "poisson_ratio",
                "dielectric_permittivity_nJ_per_V2_m",
                "mu_L_nJ_per_V_m",
                "mu_T_nJ_per_V_m",
                "mu_S_nJ_per_V_m",
        ...
        for point in points:
            writer.writerow(
                [
                    ...
                    point.metadata.mu_s,
                    (
                        point.metadata.follower_moment * MOMENT_MN_UM_TO_MICRONEWTON_UM
                        if point.metadata.follower_moment is not None
                        else None
                    ),
...
def build_metadata_annotation(points: list[PotentialPoint]) -> str:
    ...
    material_parts = [
        common_math_line(points, "young_modulus", "E", r"\mathrm{GPa}"),
    ...
    dielectric = common_math_line(
        ...
        r"\epsilon",
        r"\mathrm{nJ}/(\mathrm{V}^2\,\mathrm{m})",
    )
    if dielectric:
    ...
    follower_moment = common_math_line(
        ...
        MOMENT_MN_UM_TO_MICRONEWTON_UM,
    )
    if follower_moment:
...
def plot_points(
    ...
    ax.set_ylabel("Electrode electric potential (V)")
    ax.set_title(
...
def print_table(points: list[PotentialPoint], root: Path) -> None:
    print("length_scale/H\telectrode_potential\tlength_scale\tH\tfile")
    for point in points:
        try:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<result_folder>/electrode_potential_vs_length_scale_ratio.pdf."
        ),
    )
    parser.add_argument(
        "--csv",
...
def main() -> None:
    ...
    output = (
        ...
        else root / "electrode_potential_vs_length_scale_ratio.pdf"
    )
    linear_output = (
