...
int main(int argc, char* argv[])
{
    ...
    else
    {
        ...
        if (argc > 5) followerMoment = std::stod(argv[5]);
        ...
        if (argc > 12) initialLoadStep = std::stod(argv[12]);
        if (argc > 13) maxNewtonIterations = std::stoi(argv[13]);
        if (argc > 14) targetNumIterations = std::stoi(argv[14]);
        if (argc > 15) outputGaussPointData =
            parseBool(argv[15], "outputGaussPointData");
        if (argc > 16) outputControlPointLocations =
            parseBool(argv[16], "outputControlPointLocations");
    ...
    const std::string filenameParaview =
        "strainGradient_2DBeamBending_followerMoment_";
    const std::string outputFolder =
