...
int main(int argc, char* argv[])
{
    if (argc < 3)
    {
        std::cerr << "Usage: ./electroelasticity_2DTrapezoid <numRefinements> <numDegElev>\n";
        return 1;
    }
    ...
    double maxElePotential = 3.0;
	std::vector<int> numPointsPerPatch{ 1000 };

    int critNumIter = 20;

    if (!std::filesystem::exists("./electroelasticity_2DTrapezoid"))
		std::filesystem::create_directory("./electroelasticity_2DTrapezoid");
	std::string filenameParaview = "electroelasticity_2DTrapezoid_";
	std::string outputFolder = "./electroelasticity_2DTrapezoid/" + filenameParaview + "output";
    ...
    while (abs(appliedVoltage - maxElePotential) > 1e-10 && !endingSimulation /*&& step <= 20*/)
    {
        ...
        std::cout << "Step " << step << ": Applying voltage " << appliedVoltage << ", voltagePerStep: " << voltagePerStep << "\n";
