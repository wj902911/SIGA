...
int main(int argc, char* argv[])
{
    ...
    const double pointLoadY = argc > 3 ? std::stod(argv[3]) : -100.0;
    const double initialLoadStep = argc > 4 ? std::stod(argv[4]) : 0.1;
    const int targetNumIterations = argc > 5 ? std::stoi(argv[5]) : 5;
    if (initialLoadStep <= 0.0)
        throw std::invalid_argument("initialLoadStep must be positive.");

    const std::string rootFolder = "./2DBeamBending_cornerPointLoad_output";
    std::string outputFolder = rootFolder + "/2DBeamBending_cornerPointLoad_output";
    for (int i = 1; i < argc; ++i)
        outputFolder += "_" + std::string(argv[i]);
    std::filesystem::create_directories(outputFolder);
    TeeLogger log(outputFolder + "/log.txt");

    std::cout << "2D cantilever beam bending with corner point load\n";
    ...
    const std::string filePrefix = outputFolder + "/2DBeamBending_cornerPointLoad_";
